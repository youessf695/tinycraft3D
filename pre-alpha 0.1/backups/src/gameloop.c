...
static void create_text_texture(GLuint* tex_id, const char* text, int* w, int* h, int* pot_w, int* pot_h) {
    ...
    if (game_font == NULL || text == NULL || text[0] == '\0') return;
...
static void draw_cached_text(GLuint tex_id, float x, float y, int w, int h, int pot_w, int pot_h, SDL_Color color) {
    ...
    struct { float dx, dy; float r, g, b; } passes[5] = {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        glBegin(GL_QUADS);
        glTexCoord2f(0.0f,  0.0f);  glVertex2f(x + passes[i].dx,     y + passes[i].dy);
        glTexCoord2f(tex_u, 0.0f);  glVertex2f(x + passes[i].dx + w, y + passes[i].dy);
        glTexCoord2f(tex_u, tex_v); glVertex2f(x + passes[i].dx + w, y + passes[i].dy + h);
        glTexCoord2f(0.0f,  tex_v); glVertex2f(x + passes[i].dx,     y + passes[i].dy + h);
        glEnd();
    }
...
static void draw_highlight_cube(int x, int y, int z) {
    ...
    glBegin(GL_QUADS);
    glVertex3f(fx, fy, fz + s); glVertex3f(fx + s, fy, fz + s); glVertex3f(fx + s, fy + s, fz + s); glVertex3f(fx, fy + s, fz + s);
    glVertex3f(fx, fy, fz); glVertex3f(fx, fy + s, fz); glVertex3f(fx + s, fy + s, fz); glVertex3f(fx + s, fy, fz);
    glVertex3f(fx, fy + s, fz); glVertex3f(fx, fy + s, fz + s); glVertex3f(fx + s, fy + s, fz + s); glVertex3f(fx + s, fy + s, fz);
    glVertex3f(fx, fy, fz); glVertex3f(fx + s, fy, fz); glVertex3f(fx + s, fy, fz + s); glVertex3f(fx, fy, fz + s);
    glVertex3f(fx + s, fy, fz); glVertex3f(fx + s, fy + s, fz); glVertex3f(fx + s, fy + s, fz + s); glVertex3f(fx + s, fy, fz + s);
    glVertex3f(fx, fy, fz); glVertex3f(fx, fy, fz + s); glVertex3f(fx, fy + s, fz + s); glVertex3f(fx, fy + s, fz);
    glEnd();
    ...
    glLineWidth(2.0f);
    glBegin(GL_LINES);
    glVertex3f(fx, fy, fz); glVertex3f(fx+s, fy, fz);
    glVertex3f(fx+s, fy, fz); glVertex3f(fx+s, fy, fz+s);
    glVertex3f(fx+s, fy, fz+s); glVertex3f(fx, fy, fz+s);
    glVertex3f(fx, fy, fz+s); glVertex3f(fx, fy, fz);
    glVertex3f(fx, fy+s, fz); glVertex3f(fx+s, fy+s, fz);
    glVertex3f(fx+s, fy+s, fz); glVertex3f(fx+s, fy+s, fz+s);
    glVertex3f(fx+s, fy+s, fz+s); glVertex3f(fx, fy+s, fz+s);
    glVertex3f(fx, fy+s, fz+s); glVertex3f(fx, fy+s, fz);
    glVertex3f(fx, fy, fz); glVertex3f(fx, fy+s, fz);
...
bool init_game(void) {
    ...
    if (!load_all_textures()) return false; 

    if (TTF_Init() >= 0) {
        game_font = TTF_OpenFont("C:\\Windows\\Fonts\\arial.ttf", 20);
        if (game_font == NULL) {
            game_font = TTF_OpenFont("arial.ttf", 20); 
        }
    }
...
void run_game_loop(void) {
    ...
    while (is_running) {
        ...
        while (SDL_PollEvent(&event)) {
            if (event.type == SDL_QUIT) {
                ...
                // 💡 [ميزة كشف الضغط المزدوج على Space لتفعيل الطيران]
                    ...
                    if (event.key.repeat == 0) { // 🔥 السطر السحري: تخطي التكرار التلقائي تماماً عند تعليق الزر
                        Uint32 now = SDL_GetTicks();
                        if (now - player.last_space_time < 250) { // تم الضغط مرتين متتاليتين في أقل من ربع ثانية
        ...
        if (mouse_grabbed) handle_player_input(&player, dt);
        ...
        if (player.selected != last_selected) {
            ...
            if (player.selected == BLOCK_STONE) block_name = "Selected Block: Stone (Press 2)";
            if (player.selected == BLOCK_WOOD)  block_name = "Selected Block: Wood (Press 3)";
...
void clean_game(void) {
    if (notification_tex != 0) glDeleteTextures(1, &notification_tex);
    if (block_tex != 0) glDeleteTextures(1, &block_tex);
    if (coords_tex != 0) glDeleteTextures(1, &coords_tex);
